...
public class MostPoints2140 {
    ...
    public static void main(String[] args) {
        MostPoints2140 m = new MostPoints2140();
        int[][] questions = {{3,2},{4,3},{4,4},{2,5}};
        System.out.println(m.mostPoints(questions));
    }

//    top down recursive dp; time: O(n), space: O(n)
...
//        dp[i] = max of (solve it, skip it)
        dp[index] = Math.max(points + dfs(questions, index + skip + 1),
                dfs(questions, index + 1));
        return dp[index];
    }

//    bottom up iterative dp; time: O(n), space: O(n) [faster]
